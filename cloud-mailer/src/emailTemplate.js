import { DEFAULT_SETTINGS, DEFAULT_SUBJECT } from './db.js';
...
export function buildEmail(companyName, settings) {
  const company = fallback(companyName, '北京创谱科技有限公司');
  const branch = fallback(settings.branchName, DEFAULT_SETTINGS.branchName);
  const president = fallback(settings.presidentName, DEFAULT_SETTINGS.presidentName);
  const manager = fallback(settings.managerName, DEFAULT_SETTINGS.managerName);
  const phone = fallback(settings.managerPhone, DEFAULT_SETTINGS.managerPhone);
  const subject = fallback(settings.subject, DEFAULT_SUBJECT);
